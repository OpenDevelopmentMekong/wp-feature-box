...
function wpfbox_get_field_reference( $field_name, $post_id ) {
...
function wpfbox_get_field_objects( $post_id = false, $options = array() ) {
	...
	if( is_array($keys) )
	{
		foreach( $keys as $key )
		{
			$field = wpfbox_get_field_object( $key, $post_id, $options );
...
function wpfbox_get_fields( $post_id = false, $format_value = true ) {
	...
	$fields = wpfbox_get_field_objects( $post_id, $options );
...
function wpfbox_get_field( $field_key, $post_id = false, $format_value = true ) {
	...
	$field = wpfbox_get_field_object( $field_key, $post_id, $options);
...
function wpfbox_get_field_object( $field_key, $post_id = false, $options = array() ) {
	...
	{
		// get field key
		$field_key = wpfbox_get_field_reference( $field_key, $post_id );
	}
...
*  the_field()
*
*  This function is the same as echo wpfbox_get_field().
*
*  @type	function
*  @since	1.0.3
*  @date	29/01/13
...
function the_field( $field_name, $post_id = false ) {
	...
	$value = wpfbox_get_field($field_name, $post_id);
...
function have_rows( $field_name, $post_id = false ) {
	...
	{
		// vars
		$f = wpfbox_get_field_object( $field_name, $post_id );
		$v = $f['value'];
		unset( $f['value'] );
...
add_filter('acf/wpfbox_get_field_groups', 'api_acf_wpfbox_get_field_groups', 2, 1);
function api_acf_wpfbox_get_field_groups( $return )
{
	// validate
...
add_filter('acf/field_group/wpfbox_get_fields', 'api_acf_field_group_wpfbox_get_fields', 1, 2);
function api_acf_field_group_wpfbox_get_fields( $fields, $post_id )
{
	// validate
	if( !empty($GLOBALS['acf_register_field_group']) )
	{
...
*
*  @return	string	$value: the value found by wpfbox_get_field
*/
...
function acf_shortcode( $atts )
{
	...
	$value = wpfbox_get_field( $field, $post_id );
...
function acf_form( $options = array() )
{
	...
	if( is_array($acfs) ){ foreach( $acfs as $acf ){
		...
		// load fields
		$fields = apply_filters('acf/field_group/wpfbox_get_fields', array(), $acf['id']);
...
*  @date	17/03/13
*
*  @param	array	$value: the value returned via wpfbox_get_field

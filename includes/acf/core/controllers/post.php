...
class acf_controller_post
{
	...
	*/
	...
	function meta_box_input( $post, $args )
	{
		...
		if( $args['show'] )
		{
			$fields = apply_filters('acf/field_group/wpfbox_get_fields', array(), $args['field_group']['id']);
...
(function($) {
	...
	*/
	...
	function is_protected_meta( $protected, $meta_key, $meta_type ) {
		...
		// if acf_wpfbox_get_field_reference returns a valid key, this is an acf value, so protect it!
		if( !$protected ) {
			
			$reference = wpfbox_get_field_reference( $meta_key, $post->ID );
